...
export default function Page({ params, columns = 4, pageSize = 10 }) {
  const { id } = params || {}; // safe destructure
  const [isListView, setIsListView] = useState(true);
  const [classes, setClasses] = useState('col-lg-3 col-md-3 col-sm-6 col-12');

  const router = useRouter();
  const searchParams = useSearchParams();

  // Keep page in sync with URL query
  ...
  const handlePagination = (page) => {
    setCurrentPage(page);
    router.push(`?page=${page}`, { scroll: false }); // update query in URL
  };
